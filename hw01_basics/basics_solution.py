...
def hello_semester():
    """ Print the string 'Welcome to "Symbolische Programmierung" WS 22/23'.
    >>> hello_semester()
    Welcome to "Symbolische Programmierung" WS 22/23
    """
    print("Welcome to \"Symbolische Programmierung\" WS 22/23")
...
def swap_half(list_a):
    ...
    """
    middle = len(list_a) // 2
    return list_a[middle:] + list_a[:middle]


def replace_elements(list_a, replacement_indices, new_value):
    """ Replace all elements at the position of any element inside replacement with a new value.
...
def long_strings(string_list, max_length):
    ...
    """
    # return [True if len(s)>len(max_length) else False for s in string_list]
    return [len(s) > max_length for s in string_list]
...
def contains_substring(substring, string):
    ...
    False
    """
    return bool(re.search(substring, string, re.IGNORECASE))
